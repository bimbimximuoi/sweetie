...
def text_to_speech(text, socket_id=None):
    tts_output_path = os.path.join(os.path.dirname(__file__), '..', 'static', 'output.mp3')
    ...
    try:
        ...
        with open(tts_output_path, 'wb') as f_out:
            for i, chunk in enumerate(text_chunks):
                tts = gTTS(text=chunk, lang='en')
                chunk_output_path = os.path.join(os.path.dirname(__file__), '..', 'static', f'chunk_{i}.mp3')
                tts.save(chunk_output_path)
                
                # Append each chunk to the final output file
                with open(chunk_output_path, 'rb') as f_in:
                    f_out.write(f_in.read())
                os.remove(chunk_output_path)
                
                # Update progress (dummy implementation as gTTS does not provide progress)
                overall_progress = int(((i + 1) / total_chunks) * 100)
                if socket_id:
...
def generate_tts():
    ...
    tts_output_path = text_to_speech(text, socket_id)
